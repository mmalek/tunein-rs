...
#[test]
fn sample_1() {
    let document = opml::reader::read(File::open("tests/documents/sample_1.opml").unwrap()).unwrap();
...
#[test]
fn sample_2() {
    let document = opml::reader::read(File::open("tests/documents/sample_2.opml").unwrap()).unwrap();
    let expected_document = opml::Document {
        version: opml::Version{major: 1, minor: 0},
